...
export function parseJudgeOutput(rawText: string): ParseResult {
    ...
    if (trimmed.startsWith("{")) {
        try {
            ...
            parseErrors.push(`[strict] JSON.parse failed: ${err instanceof Error ? err.message : String(err)}`);
        }
    } else {
    ...
    if (firstBrace !== -1) {
        ...
        if (jsonCandidate !== null) {
            try {
                ...
                parseErrors.push(`[fallback] JSON.parse failed: ${err instanceof Error ? err.message : String(err)}`);
            }
        } else {

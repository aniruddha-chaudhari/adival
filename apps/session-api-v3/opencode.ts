...
export async function ensureOpencodeServer(): Promise<void> {
  if (await isHealthy(OPENCODE_BASE_URL)) {
    console.log("[api-v3] Connected to existing OpenCode server");
    return;
  }

  console.log(`[api-v3] Starting OpenCode server on ${OPENCODE_HOST}:${OPENCODE_PORT}...`);
  ...
  for (let attempt = 0; attempt < 60; attempt += 1) {
    await sleep(100);
    if (await isHealthy(OPENCODE_BASE_URL)) {
      console.log("[api-v3] OpenCode server is ready");
      return;
    }
  }

  throw new Error(`Failed to connect to OpenCode server on ${OPENCODE_HOST}:${OPENCODE_PORT}`);
}
